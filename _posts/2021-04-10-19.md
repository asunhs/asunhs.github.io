...
쌩뚱맞은 도입이지만 좋은 코드란 무엇일까요? 여러 의견들이 있지만 그 중 꽤 많이 거론되는 덕목은 변경하기 용이한 코드입니다.
코드의 작성 시점에서 변경은 미래에 일어날 일이고 지금 그 요구를 다 안다면 기다릴 것 없이 코드에 반영하면 되지만 그렇지 않기 때문에 변경이 용이할 수 있다는 점은 쉽지는 않습니다.
한 편으로는 그렇기에 코드를 변경하게 되는 케이스들을 일반화하여 코드를 작성하는 시점에 어떤 변경 패턴애 대비하곤 합니다.
...
먼저, 코드를 변경하는 패턴 중에는 코드가 진행하는 흐름을 사용한 패턴을 짧게 살펴볼까 합니다. 대표적으로 `then` 의 의미를 사용하는 방식들을 찾아볼 수 있습니다.
...
```java
public class MiddlewareTask<T> {

  private final BiFunction<T, UnaryOperator<T>, T> middleware;
  // ...
```

middleware 는 T 타입의 파라미터와 `next` 제어함수를 받아 역시 T 타입의 결과를 반환하는 형태의 함수입니다.
...
public class MiddlewareTask<T> {
  ...
  private MiddlewareTask(BiFunction<T, UnaryOperator<T>, T> middleware) {
    this.middleware = middleware;
  }

  public static <T> MiddlewareTask<T> of(BiFunction<T, UnaryOperator<T>, T> middleware) {
...
```java
 // ...
  public T run(T param) {
    return middleware.apply(param, identity());
  }
}
```
...
public class MiddlewareTask<T> {
  ...
  public MiddlewareTask<T> andThen(MiddlewareTask<T> after) {
...
public class MiddlewareTask<T> {
  ...
  public MiddlewareTask<T> andThen(BiFunction<T, UnaryOperator<T>, T> middleware) {
...
직관적인 사용을 위해 `andThen(BiFunction<T, UnaryOperator<T>, T>)` 을 추가 하였습니다.
...
private MiddlewareTask<String> greeter = 
    MiddlewareTask.of(this::extract)
        .andThen((value, next) -> "hello " + value)

public String greeting(String record) {
  return greeter.run(record);
}
...
private String crypto(String value, UnaryOperator<String> next) {
  final String key = "_KeY_"; // decrypt/encrypt 에 동일한 키 사용
...
사실 middleware monad 는 업무에 필요한 순간이 생겨서 고려하게 되었습니다. [Try Catch 지옥 벗어나기](/20) 에서 실 사용 사례를 소개해보도록 하겠습니다.
...
이번 예시와 [Try Catch 지옥 벗어나기](/20) 를 통해 제가 Monad 를 설명해 본 방식이 효과적이길 바래봅니다.
